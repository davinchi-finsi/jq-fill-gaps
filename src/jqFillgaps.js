...
$.widget("hz.fillgaps", {
    ...
    _buildHtml:function () {
        ...
        if( this._numberGaps ==0) {
            ...
            for (let i = 0; i < _gaps.length; i++) {
                ...
                // añadimos las clases
                $(_gaps[i]).addClass('ui-droppable hz-fill-gaps-gap_destiny');
                // añadimos el id
                $(_gaps[i]).attr('data-hz-fill-gaps-gap_destiny', i);
                // añadimos la longitud de la palabra por si queremos hacer el ancho del hueco en función de la misma
                $(_gaps[i]).attr('data-hz-fill-gaps-gap_lenght', _gaps[i].innerText.length);
                // pintamos dentro la interrogación
                $(_gaps[i]).html('<span class="gap_empty">?</span>');
            }
   ...
   _drawWords:function () {
     ...
     _createEvents:function() {
         ...
         $('.hz-fill-gaps-gap_origin').draggable(
             {
                 revert: "valid",
         ...
         $('.hz-fill-gaps-gap_destiny').droppable({
    ...
    _handleDrop:function (event, ui, _this) {
        ...
        if(that._numberGapsOK==that._numberGaps){
            ...
            that.element.find('.hz-fill-gaps-gaps_origin').remove();
        }
    },
    ...
    _handleGapFilled:function (origin) {
        ...
        _gaps_ko.on('click', function() {
            word=$(this).html();
            $(this).removeClass('hz-gap-filled hz-gap-evaluate-ko');
            $(this).addClass('gap_empty').html('?');
